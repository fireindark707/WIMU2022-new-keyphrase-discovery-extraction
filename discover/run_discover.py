...
documents = [a for a in documents if a==a]

for idx,doc in enumerate(documents):
    documents[idx] = re.sub(r'''[⇙▼★✦➤♥✪!"#\'()*+,-:;<=>?@\[\]^_`|}~·×ΔΨγμφ—‘’“”℃Ⅲ↓↑→▍▌∈❶❷❸❹①②③④⑤⑥⑦⑧⑨⑩©■▲、。〈〉《》」『』【】〔〕
    ㈧︿！＃＇（）＊＋，－β．•●：；＜Δφ＝＞><λ＠＿｛｜｝～￥]''',' ',doc)

#for doc in documents[:3]:
#    print(doc)

corpus_name = args.input_path.split(".")[0]
...
for iteration in range(args.iteration):
   ...
   **********************************************************************
    ...
    '''
    print("say something")
    print(discoverer.char_counter)
    print(discoverer.nr_of_chars)
    '''
